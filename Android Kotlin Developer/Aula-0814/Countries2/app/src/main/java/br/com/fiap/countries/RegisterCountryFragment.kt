    ...
    private fun insertUpdateData() {
        binding.run {
            ...
            if (countryInfoArgument == null) {
                ...
                showRegisterMessage(getString(R.string.register_country_success_registered_message,
                    countryModel.name
                ))
                clearForm()
            } else {
                appDb?.countryDAO()?.update(countryModel)
                showRegisterMessage(getString(R.string.register_country_success_updated_message,
                    countryModel.name
                ))
            }
        }
